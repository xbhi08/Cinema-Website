...
<?php
   /* require_once "includes/db_connect.php";
    ...
    $sQuery = "SELECT * FROM shows WHERE showID = ".$movieID;
    ...
    if($userResults['showID'] )//the user exists
    {	
...
<head>
    ...
    $(document).ready(function(){
        ...
        //$("a.poster").click(function(){
            ...
            var show_id = $(this).attr('id');  //id of film poster chosen
            //alert(show_id);

           /* $.post( "showID.php", {showID: show_id}, function(data){
                if(status == "success"){
            ...
            $.ajax({
                url:"setSessionToShowID.php",  //tranfer control to this page to set session to showID of obj referenced
                data: {showID: show_id}, 
                cache: false,
                method: "POST",
                success: function(data){
                    //alert("In1 ");
                    location.href = "result.php";  //equivalent to header loc
                }, 
                error: function(xhr){
        ...
        <div class="container mt-5">
            ...
            <section class="bl">
                <div class="heading">
                    <h2>Recommended</h2>
                    <div class="tabs">
                        <span id="playing" data-name="playing now" class="active"><i class="fas fa-play-circle"></i> Playing Now</span>
                        <span id="trending" data-name="trending"><i class="fas fa-chart-line"></i> Trending</span>
                        <span id="upcoming" data-name="upcoming"><i class="fas fa-list"></i> Upcoming</span>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="content">
                    <div class="tab-content" data-name="playing now">
                        ...
                        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        $sql = 'SELECT shows.show_name, shows.duration, shows.imdb_rating, shows.quality, shows.show_desc, shows.show_url, shows.show_year, shows.showID
                        FROM shows 
                        ...
                        <div class="filmlist "> <!--films can be arranged in rows(i.e filling the rows)-->
                            ...
                            <?php
                                ...
                                { 
                                    ...
                                    echo    '<a  title="'.$row['show_name'].'" class="poster" id="'.$row['showID'].'">
